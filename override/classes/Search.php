...
class Search extends SearchCore
{
    ...
    */
		...
		$order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
	{
		...
		INNER JOIN `'._DB_PREFIX_.'category` c ON cp.`id_category` = c.`id_category`'.(Tools::getValue('category_filter')
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, 
				...
				LIMIT '.(((int)$page_number - 1) * (int)$page_size).','.(int)$page_size;
		...
		if ($ajax)
		{
			...
			{
				$res['image'] = $context->link->getImageLink($res['link_rewrite'], $res['id_image'], ImageType::getFormatedName('medium'));
				if (!Configuration::get('PS_CATALOG_MODE') && ((isset($res['show_price']) && $res['show_price'])
								|| (isset($res['available_for_order']) && $res['available_for_order'])))
					if (isset($res['show_price']) && $res['show_price'])
						if (!$price_display)
