...
{foreach from=$rows item=row name=row}
    ...
    {/if}    
    <div class="row row-level-{$row->level|escape:'html':'UTF-8'}"><div class="row-inner {if isset($row->sfxcls)&&$row->sfxcls}pts-row {$row->sfxcls|escape:'html':'UTF-8'} {/if} {if $row->level>=2}{$row->cls|escape:'html':'UTF-8'}{/if} clearfix" >
        {foreach $row->cols as $col}
            <div class="col-lg-{$col->lgcol|escape:'html':'UTF-8'} col-md-{$col->mdcol|escape:'html':'UTF-8'} col-sm-{$col->smcol|escape:'html':'UTF-8'} col-xs-{$col->xscol|escape:'html':'UTF-8'}">
               ...
               {if isset($row->parallax)&&$row->parallax}
                ...
                {/if}
                {/if}
                    <div class="col-inner {if isset($col->sfxcls)&&$col->sfxcls}pts-col {$col->sfxcls|escape:'html':'UTF-8'}{/if} {$col->cls|escape:'html':'UTF-8'}" {$col->attrs}{* HTML, cannot escape *}>
